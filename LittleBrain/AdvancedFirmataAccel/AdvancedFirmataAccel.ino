...
#define STEPPER_SETACCEL    2
#define STEPPER_SETMAXSPEED    3
#define STEPPER         0x08  // pin configured for stepper motor
#define STEPPER_PROGRESS    0x73 
#define LED_STRIP_SETUP 0x74
#define LED_STRIP_SETCOLOR 0x75
...
unsigned int stepperProgressInterval = 200; // how often to run the main loop (in ms)
unsigned long previousStepProgressMillis = 0;       // for comparison with currentMillis
...
byte neckDirectionPin = 12;
byte neckPWMPin = 3;
byte selfieDirectionPin = 13;
byte selfiePWMPin = 11;

byte neckStop = 14;
byte selfieStop = 15;

byte stepperLeftEnable = 5;
byte stepperRighEnable = 8;
...
void sysexCallback(byte command, byte argc, byte *argv)
{
  ...
  Serial1.print("Processing Sysex Command: ");
  ...
  case STEPPER_DATA:
    ...
    if (deviceNum < MAX_STEPPERS)
    {
      if (stepCommand == STEPPER_CONFIG)
      {
        ...
        {
          stepper[deviceNum] = new AccelStepper(1, stepPin, directionPin);
        }
        else if (interfaceType == 4)
        {
          ...
          stepper[deviceNum] = new AccelStepper(4, stepPin, directionPin, motorPin3, motorPin4);
        }
      }
      else if (stepCommand == STEPPER_STEP)
      {
        ...
        if (stepper[deviceNum])
        {
          if (argc >= 8 && argc < 12)
          {
            stepper[deviceNum]->setMaxSpeed(stepSpeed);
        ...
        else if (stepCommand == STEPPER_SETACCEL)
        {
          int accel = (argv[2] + (argv[3] << 7));
          stepper[deviceNum]->setAcceleration(accel);

        }
        else if (stepCommand == STEPPER_SETMAXSPEED)
        {
          int stepSpeed = (argv[2] + (argv[3] << 7));
          stepper[deviceNum]->setMaxSpeed(stepSpeed);
        }
      }
    }
...
void setup()
{
  pinMode(neckStop, INPUT_PULLUP);
  pinMode(selfieStop, INPUT_PULLUP);
  pinMode(neckPWMPin, OUTPUT);
  pinMode(selfiePWMPin, OUTPUT);
  pinMode(stepperLeftEnable, OUTPUT);
  pinMode(stepperRighEnable, OUTPUT);

  analogWrite(neckPWMPin, 0);
  analogWrite(selfiePWMPin, 0);

  // Disable steppers on startup for safety
  // Enable low
  digitalWrite(stepperLeftEnable, HIGH);
  digitalWrite(stepperRighEnable, HIGH);
...
void loop()
{
  ...
  //checkDigitalInputs();
  /* SERIALREAD - processing incoming message as soon as possible, while still
  ...
  if (numSteppers > 0)
  {
    for (int i = 0; i < MAX_STEPPERS; i++)
    {
      if (stepper[i])
      {
        bool running = stepper[i]->run();
        currentMillis = millis();
        if (running && 
            currentMillis - previousStepProgressMillis > stepperProgressInterval)
        {
          previousStepProgressMillis = currentMillis;
          Serial.write(START_SYSEX);
          Serial.write(STEPPER_PROGRESS);
          Serial.write(i);
          Serial.write(stepper[i]->progress());
          Serial.write(END_SYSEX);
        }
        
        // send command to client application when stepping is complete
        // If it was running, but now at target position, we fire a done command.
        if (running && (stepper[i]->targetPosition() == stepper[i]->currentPosition()))
        {
          Serial.write(START_SYSEX);
...
/*
  // Neck stop is currently unused.
  // Stop the neck.
  if (digitalRead(neckDirectionPin) == LOW &&
    digitalRead(neckStop) == LOW)
  {
    analogWrite(neckPWMPin, 0);
  }

  // Stop the neck.
  if (digitalRead(selfieDirectionPin) == LOW &&
    digitalRead(selfieStop) == LOW)
  {
    analogWrite(selfiePWMPin, 0);
  }
*/
#ifdef USE_FIRMATA_ANALOG
  currentMillis = millis();
